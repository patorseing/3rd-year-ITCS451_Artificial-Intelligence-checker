		  ...
		  for (int i = 0; i < BOARD_HEIGHT ; i++){
				for (int j = 0; j < BOARD_WIDTH; j++) {
					...
					switch(piece) {
						case RED_PAWN:
							redPawnC++;
							redChance+= Math.abs(8  - (BOARD_WIDTH*BOARD_HEIGHT) - (10/((j+1)/BOARD_WIDTH)));
							break;
						case BLACK_PAWN:
							blackPawnC++;
							blackChance+=(10/(((BOARD_WIDTH*BOARD_HEIGHT)-i)/BOARD_WIDTH));
							break;
						case RED_KING:
							redKingC++;
							redking.add(piece);
							redKCenter+= Math.abs(8  - ((BOARD_WIDTH*BOARD_HEIGHT) - (16/Math.abs(i-24)))*10);
							break;
						case BLACK_KING:
							blackKingC++;
							blackking.add(piece);
